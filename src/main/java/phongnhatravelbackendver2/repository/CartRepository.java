...
public interface CartRepository extends JpaRepository<CartEntity, Long> {
	...
	@Query(value = "SELECT c.id, c.booking_date, c.quantity, c.checkouts_id, c.tours_id, c.users_id FROM cart AS c, "
			+ "checkouts AS ck WHERE c.tours_id = :tourId and c.users_id = :userId AND (c.checkouts_id IS NULL OR "
			+ "(c.checkouts_id = ck.id AND ck.status = 'unpaid')) GROUP BY c.id, c.booking_date, c.quantity, c.checkouts_id, "
			+ "c.tours_id, c.users_id", nativeQuery = true)
